    ...
    testCases.forEach(({ name, prompt, expectedContent, timeout }) => {
        it(`should handle ${name}`, async () => {
            ...
            ocProcess.on("exit", (code) => {
                exitCode = code;
                console.log(`[TEST] opencode process exited with code ${code} for case: ${name}`);
            });
            ...
            const dashboardUrl = await new Promise<string>((resolve, reject) => {
                const checkInterval = setInterval(() => {
                    if (exitCode !== null && exitCode !== 0) {
                        ...
                        console.error(`[TEST FAIL] Process exited early with code ${exitCode}. Full log output:\n${fullOutput}`);
            ...
            try {
                ...
                const agentTab = await Promise.race([
                    ...
                    new Promise((_, reject) => {
                        const check = setInterval(() => {
                            if (exitCode !== null) {
                                clearInterval(check);
                                reject(new Error(`Tournament process exited with code ${exitCode} while waiting for agents.`));
                            }
                        }, 500);
                    })
                ]) as any;
                ...
                console.error(`[TEST FAIL] Failure while waiting for agent tab (${name}). Full log output:\n${fullOutput}`);
                 throw e;
            }

            // If we have expected content, verify it
            if (expectedContent.length > 0) {
                await page.waitForSelector(".terminal-container.active .xterm-rows", { timeout: 20000 });
                ...
                if (!found) {
                     console.error(`[TEST FAIL] Terminal content mismatch for case: ${name}. Full log output:\n${fullOutput}`);
                }
                expect(found, `Expected terminal to contain one of [${expectedContent}] but got: ${terminalText.substring(0, 200)}...`).toBe(true);
            }
