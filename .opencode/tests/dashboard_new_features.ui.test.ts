...
describe("Dashboard New Features UI", () => {
    ...
    it("should switch between System and Agent tabs", async () => {
        ...
        const agentTabs = Array.from(document.querySelectorAll(".tab-button")).filter(
        ...
        expect(document.getElementById("terminal-agent_0")?.classList.contains("active")).toBe(
            true,
        );
        expect(document.getElementById("terminal-system")?.classList.contains("active")).toBe(
            false,
        );
