...
export class TerminalManager {
    ...
    constructor(
        private io: Server,
        opts?: { verbose?: boolean; log?: LogFn },
    ) {
    ...
    public spawnTerminal(agentId: string, attempt: number = 1) {
        ...
        if (this.verbose)
            this.log("info", `[TERMINAL] Spawning helper for ${agentId} (Attempt ${attempt}/5)`);
        ...
        const child = spawn(
            "node",
            [helperPath, opencodeBin, targetUrl, meta.session_id, meta.arena_dir || process.cwd()],
            {
        ...
        child.stdout!.on("data", (chunk: Buffer) => {
            ...
            try {
                ...
                if (this.verbose)
                    this.log("info", `[TERMINAL][${agentId}] Helper ready on port ${info.port}`);
                ...
                if (this.verbose)
                    this.log(
                        "error",
                        `[TERMINAL][${agentId}] Failed to parse helper output: ${stdoutBuf}`,
                    );
            }
        });
        ...
        child.on("exit", (code, signal) => {
            ...
            if (this.verbose) {
                this.log(
                    "info",
                    `[TERMINAL-EXIT][${agentId}] Helper exited: code=${code}, signal=${signal}, duration=${duration.toFixed(1)}s`,
                );
            }
            ...
            if (duration < 3 && attempt < 5) {
                if (this.verbose)
                    this.log(
                        "info",
                        `[TERMINAL] ${agentId} helper failed quickly, retrying in 2s...`,
                    );
