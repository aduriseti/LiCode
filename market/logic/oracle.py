...
class Oracle:
    ...
    @staticmethod
    def _setup_sandbox(candidate_dir: str, verifier_dir: str) -> str | None:
        ...
        try:
            ...
            def ignore_agent_home(path, names):
                return (
                    [".home", "__pycache__", ".git"]
                    if any(x in names for x in [".home", "__pycache__", ".git"])
                    else []
                )
    ...
    async def run_test(candidate_dir: str, verifier_dir: str, timeout: int = 15) -> ResultType:
        ...
        try:
            ...
            logging.info(
                f"Oracle: Starting test {os.path.basename(verifier_dir)} on worktree {os.path.basename(candidate_dir)}"
            )
            ...
            try:
                ...
                logging.info(
                    f"Oracle: Finished test {os.path.basename(verifier_dir)} on {os.path.basename(candidate_dir)}"
                )
