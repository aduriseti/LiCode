...
class TestWhale(unittest.TestCase):
    def setUp(self):
        ...
        self.state.assets["v_high"] = MarketAsset(
            id="v_high", type="VERIFIER", description="Good Test", q_yes=460
        )

        # V2: Low validity (Market thinks it's garbage)
        # P ~ 0.01
        self.state.assets["v_low"] = MarketAsset(
            id="v_low", type="VERIFIER", description="Bad Test", q_yes=-460
        )
