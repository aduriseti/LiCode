...
class Orchestrator:
    def __init__(self, prompt: str, n_agents: int, budget: float = 1000.0, **kwargs):
        ...
        # Set permissions: 0o711 for traversal, 0o700 for individual lockdown
        self.worktrees_dir.chmod(0o711)
        ...
        if state:
            ...
            self.state = MarketState(
                round_num=0,
                liquidity_b=self.budget / 20.0,  # Initial liquidity
                whale_wealth=self.budget,
                prompt=prompt,
            )

        self.inference_tax = (0.01 * self.budget) / n_agents if n_agents > 0 else 1.0
        ...
        for i in range(self.n_agents):
            ...
            if aid not in self.state.agents:
                self.state.agents[aid] = AgentPortfolio(
                    agent_id=aid, wealth=self.budget / self.n_agents
                )
            # Initialize Candidate for each agent
            cid = f"cand_{i}"

            # Create worktree for candidate (Full Clone)
            cand_dir = self.worktrees_dir / cid

            async def setup_cand(c_dir, c_id, a_id):
                # Run async cloning
                await self._clone_workspace(str(c_dir))
                ...
                self.state.assets[c_id] = MarketAsset(
                    ...
                    code_path=str(c_dir),  # Point to ROOT of worktree
                    q_no=q_no,
                )
    ...
    async def _clone_workspace(self, dest_dir: str):
        ...
        try:
            ...
            tar_cmd = (
                f"git ls-files -co --exclude-standard {exclude_args_git} -z | "
                f"tar -c --null {exclude_args_tar} -T - | tar -x -C {dest_dir}"
            )
            proc = await asyncio.create_subprocess_shell(
                tar_cmd, cwd=src, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            await proc.communicate()
            ...
            if await asyncio.to_thread(os.path.exists, dest_dir):
                ...
                logging.warning(
                    f"Skipping baseline.diff and permissions: {dest_dir} not found (mocked clone?)"
                )
    ...
    async def process_round(self, actions: list[AgentAction]):
                        ...
                        if source_path and candidate.code_path:
                            ...
                            if os.path.isdir(full_source):
                                ...
                                if vid and vid not in frozen_state.assets:
                                    ...
                                    logging.debug(
                                        f"Agent {agent_id} proposed existing verifier {vid} (skipping redundant bond)"
                                    )
                            else:
                                logging.warning(
                                    f"Agent {agent_id} proposed verifier at non-existent path: {source_path}"
                                )
        ...
        for action in actions:
            ...
            if unified_beliefs:
                ...
                wagers = Strategy.beliefs_to_wagers(
                    unified_beliefs, frozen_state.agents[agent_id].wealth, frozen_state
                )
            ...
            if agent_id in self.state.agents:
                ...
                for vid in vids:
                    ...
                    if q_shares > 0:
                        ...
                        logging.info(
                            f"Recorded bond for {agent_id} on {vid}: {q_shares:.2f} shares"
                        )
        ...
        for aid, agent in self.state.agents.items():
            ...
            if total_net_worth <= self.inference_tax:
                logging.info(
                    f"Agent {aid} went bankrupt! (Net Worth: {total_net_worth:.2f}, Tax: {self.inference_tax:.2f})"
                )
            ...
            logging.info(
                f"Agent {aid} Wealth: {agent.wealth:.2f} (Net Worth: {total_net_worth:.2f}, Delta: {delta:+.2f})"
            )
    ...
    def _liquidate_shares(self, agent_id: str, asset_id: str, q_shares: float):
        ...
        if abs(new_shares) < 1e-5:
            if asset_id in agent.shares:
                del agent.shares[asset_id]
        else:
            agent.shares[asset_id] = new_shares

        logging.info(
            f"Liquidated {q_shares:.2f} of {asset_id} for {agent_id}. Payout: {payout:.2f}"
        )
    ...
    def _settle_all_bets(self):
        ...
        for bond in self.state.bonds:
            ...
            locked_shares[bond.agent_id][bond.asset_id] = (
                locked_shares[bond.agent_id].get(bond.asset_id, 0.0) + bond.q_shares
            )
            ...
            for asset_id, q_shares in list(agent.shares.items()):
                ...
                if abs(exposure) > 0.0001:
                    ...
                    payout = LMSRMarket.calculate_payout(
                        exposure, asset.q_yes, asset.q_no, self.state.liquidity_b
                    )
                    ...
                    self.state.whale_shares[asset_id] = (
                        self.state.whale_shares.get(asset_id, 0.0) + exposure
                    )
                    ...
                    if abs(payout) > 0.0001:
                        logging.info(
                            f"Settlement: Agent {aid} received {payout:.2f} payout for {asset_id} position ({exposure:.2f} shares)"
                        )
    ...
    def _execute_wager_batch(self, intents: list[tuple[str, str, float]]):
        ...
        for asset_id, trades in asset_intents.items():
            ...
            if rem_w_yes > 1e-5:
                delta_q_lmsr_yes = LMSRMarket.calculate_delta_q(
                    asset.q_yes, asset.q_no, b, rem_w_yes, True
                )
                asset.q_yes += delta_q_lmsr_yes
            elif rem_w_no > 1e-5:
                delta_q_lmsr_no = LMSRMarket.calculate_delta_q(
                    asset.q_yes, asset.q_no, b, rem_w_no, False
                )
            ...
            for trader_id, wager in trades:
                ...
                if wager > 0:
                    ...
                    if trader_id == "whale":
                        self.state.whale_shares[asset_id] = (
                            self.state.whale_shares.get(asset_id, 0.0) + shares_won
                        )
                    else:
                        trader_agent = self.state.agents.get(trader_id)
                        if trader_agent is not None:
                            trader_agent.shares[asset_id] = (
                                trader_agent.shares.get(asset_id, 0.0) + shares_won
                            )
                else:
            ...
            logging.info(
                f"Batch Executed [{asset_id}]: W_yes={w_yes:.2f}, W_no={w_no:.2f}, Q_match={q_match:.2f}, P_after={p_after:.4f}"
            )

    def _create_verifier_from_path(self, agent_id: str, source_path: str) -> str | None:
        ...
        self.state.assets[vid] = MarketAsset(
            id=vid, type="VERIFIER", description=f"Verifier by {agent_id}", test_path=v_dir
        )

        return vid

    def _create_verifier(self, agent_id: str, proposal: dict) -> str | None:
        ...
        if code and not files:
            files = {"test.py": code, "run.sh": "#!/bin/bash\npython3 test.py\n"}
    ...
    async def _run_oracle(self):
        ...
        for v in verifiers:
            for c in candidates:
                if not v.test_path or not c.code_path:
                    continue
    ...
    def get_winner_id(self) -> str | None:
        ...
        if len(tied_candidates) > 1:
            ...
            valid_verifiers = [
                vid
                for vid, a in self.state.assets.items()
    ...
    def get_final_report(self) -> str:
        ...
        if winner.code_path and os.path.isdir(winner.code_path):
            try:
                ...
                subprocess.run(
                    "git add .", shell=True, check=True, cwd=winner.code_path, capture_output=True
                )
                ...
                if not baseline_commit:
                    lines.append(
                        "_Failed to generate diff: Could not find 'Initial Baseline' commit._\n"
                    )
                    return "\n".join(lines)

                # Remove problem.md from staging so it's not in the diff
                subprocess.run(
                    ["git", "reset", baseline_commit, "problem.md"],
                    cwd=winner.code_path,
                    capture_output=True,
                )
                ...
                if not result.stdout.strip():
                            ...
                            if current_file_diff:
                                if len(current_file_diff) > 100:
                                    ...
                                    processed_diff.append(
                                        f"... (Truncated {len(current_file_diff) - 100} lines. Use 'git diff' to see full changes) ..."
                                    )
                                else:
                    ...
                    lines.append(
                        "> **Note:** Large diffs are truncated. To view the full diff, run:\n> `cd "
                        + winner.code_path
                        + " && git diff --cached "
                        + baseline_commit
                        + "`\n"
        ...
        lines.append(
            "> If new tests were validated, save them to your `tests/` directory (e.g., `tests/test_tournament.py`) to prevent regression."
        )
