...
async def test_agent_terminal_connection():
    ...
    subprocess.run(
        ["git", "config", "user.name", "Test User"], cwd=temp_dir, check=True, capture_output=True
    )
    subprocess.run(
        ["git", "config", "commit.gpgsign", "false"], cwd=temp_dir, check=True, capture_output=True
    )
    subprocess.run(["git", "commit", "-m", "init"], cwd=temp_dir, check=True, capture_output=True)

    process = await asyncio.create_subprocess_exec(
        *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, env=env, cwd=temp_dir
    )
        ...
        async with async_playwright() as p:
            ...
            while (time.time() - start_time) < 45:
                terminal_text = await page.evaluate(
                    "() => { const rows = document.querySelector('.terminal-container.active .xterm-rows'); return rows ? rows.textContent : ''; }"
                )
            ...
            await browser.close()
            assert found, (
                f"Target content not found in UI terminal within 45s. Got: {terminal_text[:200]}"
            )
...
async def test_real_dashboard_startup():
    ...
    process = await asyncio.create_subprocess_exec(
        *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=temp_dir, env=env
    )
    ...
    try:
        ...
        try:
            await asyncio.wait_for(asyncio.gather(read_stdout(), read_stderr()), timeout=40)
        except asyncio.TimeoutError:
            pass
        ...
        async with aiohttp.ClientSession() as session:
            ...
            arena_dirs = [
                d for d in os.listdir(os.path.join(temp_dir, ".arenas")) if d.startswith("run_")
            ]
            ...
            for _ in range(30):  # Wait up to 30s for AI to think and act
                ...
                if stream_traces:
                    with open(os.path.join(traces_dir, stream_traces[0])) as f:
                        content = f.read()
            ...
            if process.returncode is not None:
                ...
                if process.returncode not in [0, -15, 143]:
                    pytest.fail(
                        f"Dashboard exited prematurely with code {process.returncode} after client disconnected."
                    )
    finally:
        # 3. Cleanup
        try:
