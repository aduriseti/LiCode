...
from market.core.state import MarketAsset, MarketBond, MarketState
...
class TestOrchestrator(unittest.IsolatedAsyncioTestCase):
    async def test_basic_loop(self):
        ...
        action_0 = AgentAction(agent_id="agent_0", beliefs={"cand_0": 0.7})
    ...
    async def test_whale_intervention(self):
        ...
        orch.state.assets["v1"] = MarketAsset(
            id="v1", type="VERIFIER", description="Manual V", q_yes=100
        )  # Price > 0.5
    ...
    async def test_no_overspend_during_round(self):
        ...
        action = AgentAction(
            agent_id="agent_0", beliefs={"cand_0": 0.99, "cand_1": 0.01, "cand_2": 0.99}
        )
...
class TestBondLogic(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.state = MarketState(round_num=0, liquidity_b=100.0, whale_wealth=2000.0, prompt="test")
    ...
    def test_bond_lifecycle(self):
        ...
        wagers = Strategy.beliefs_to_wagers(
            {vid: 0.99}, self.state.agents["agent_0"].wealth, self.state
        )
    ...
    async def test_instant_settlement(self):
        ...
        action = AgentAction(
            "agent_0", beliefs={"cand_0": 0.6}, proposals=[{"type": "VERIFIER", "path": "v1"}]
        )
...
class TestOrchestratorVerifier(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.test_dir = tempfile.mkdtemp()
        self.orchestrator = Orchestrator(prompt="test prompt", n_agents=1, base_dir=self.test_dir)
    ...
    async def test_clone_workspace_adds_exclude_list(self):
        ...
        with open(os.path.join(src_repo, "file.txt"), "w") as f:
            f.write("test")
        subprocess.run(["git", "add", "file.txt"], cwd=src_repo, check=True, capture_output=True)
        subprocess.run(
            ["git", "commit", "-m", "init"], cwd=src_repo, check=True, capture_output=True
        )
...
class TestNewFeatures(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bond_deduplication(self):
        ...
        self.assertEqual(
            ...
            0,
            "Redundant bond should not be created for existing verifier",
        )
