...
class TestMarketRunner(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_loop_basics(self, MockPopen, MockSocket, MockShark):
        ...
        async def test_convergence_bankruptcy(self, MockSocket, MockShark):
            ...
            runner.orchestrator.state.agents["agent_0"] = AgentPortfolio(
                agent_id="agent_0", wealth=4.5
            )
            runner.orchestrator.state.agents["agent_1"] = AgentPortfolio(
                agent_id="agent_1", wealth=4.5
            )
    ...
    def test_convergence_stability(self, MockShark):
        ...
        runner.orchestrator.state.agents["agent_0"] = AgentPortfolio(
            agent_id="agent_0", wealth=500.0
        )
        runner.orchestrator.state.agents["agent_1"] = AgentPortfolio(
            agent_id="agent_1", wealth=500.0
        )
        runner.orchestrator.state.assets["cand_0"] = MarketAsset(
            id="cand_0", type="CANDIDATE", description="Test"
        )
        runner.orchestrator.state.assets["cand_1"] = MarketAsset(
            id="cand_1", type="CANDIDATE", description="Test"
        )
    ...
    @patch("market.runner.asyncio.open_connection")
    @patch("market.runner.Shark")
    async def test_dashboard_script_path_resolution(
        self, MockShark, MockAsyncSocket, MockExec, MockClientSession
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                runner = MarketRunner(
                    "Test", n_agents=1, budget=100.0, api_url="http://127.0.0.1", dashboard=True
                )
                ...
                self.assertTrue(
                    dash_script.endswith(os.path.join(".opencode", "lib", "dashboard-server.ts"))
                )
                self.assertFalse(
                    dash_script.startswith(temp_dir),
                    "Dashboard path was incorrectly resolved relative to CWD",
                )
            finally:
    ...
    @patch("market.runner.asyncio.open_connection")
    @patch("market.runner.Shark")
    async def test_dashboard_agent_registration(
        self, MockShark, MockAsyncSocket, MockSubprocess, MockClientSession
    ):
