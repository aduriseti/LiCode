...
from market.cli import UnbufferedStreamHandler, WrappingFormatter
...
class TestLogging(unittest.TestCase):
    def test_wrapping_formatter_indentation(self):
        ...
        formatter = WrappingFormatter(
            fmt="%(asctime)s - %(levelname)s - %(message)s", datefmt="%H:%M:%S", width=30
        )
        ...
        for i in range(1, len(lines)):
            self.assertTrue(
                lines[i].startswith(expected_indent),
                f"Line {i} not indented correctly: '{lines[i]}'",
            )
