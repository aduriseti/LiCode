    ...
    def format(self, record):
        ...
        if len(parts) >= 3:
            ...
            wrapped_body = textwrap.fill(
                body, width=self.width, subsequent_indent=" " * len(header)
            )
...
def main():
    ...
    run_parser.add_argument(
        ...
        type=float,
        default=120.0,
        help="Initial agent response timeout in seconds (doubles on each retry)",
    )
    run_parser.add_argument(
        "--max-retries",
        type=int,
        default=2,
        help="Max timeout retries per round (default 2 results in 3 attempts: 2m, 4m, 8m)",
    )
    run_parser.add_argument(
        "--initial-backoff", type=float, default=120.0, help="Deprecated: use --timeout instead"
    )
    run_parser.add_argument(
        "--max-backoff", type=float, default=1000.0, help="Maximum timeout ceiling in seconds"
    )
    ...
    run_parser.add_argument(
        "--json-logs", action="store_true", help="Output JSON logs to stdout instead of TUI"
    )
    run_parser.add_argument(
        "--dashboard", action="store_true", help="Launch and log to the local web dashboard"
    )
    run_parser.add_argument(
        "--log-level",
        type=lambda x: x.upper(),
    ...
    if args.command == "init":
        ...
        async def run_tournament():
            try:
                ...
                await runner.run_loop(
                    args.rounds, stream_ui=not args.json_logs, json_logs=args.json_logs
                )
            finally:
        ...
        output = {"state": json.loads(runner.orchestrator.state.to_json()), "report": report}
