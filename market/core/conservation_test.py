...
class TestWealthConservation(unittest.IsolatedAsyncioTestCase):
    async def test_zero_sum_conservation_extended(self):
        ...
        orch.state.assets["v_manual"] = MarketAsset(id="v_manual", type="VERIFIER", description="Manual", q_yes=10.0)
        
        actions_r2 = [
            AgentAction("agent_0", beliefs={"cand_0": 0.5}), # Agent 0 changes mind
        ]
        await orch.process_round(actions_r2)
        self.assertAlmostEqual(get_total_wealth(), expected_total, places=5, msg="Leaked in R2 (Whale Intervene)")
        ...
        actions_r3 = [
            AgentAction("agent_0", proposals=[{"type": "VERIFIER", "path": "v_bond"}])
        ]
        ...
        self.assertAlmostEqual(get_total_wealth(), expected_total, places=5, msg="Leaked in R3 (Bond Create)")
        
        # Round 4: Bond Maturation
        await orch.process_round([])
        self.assertAlmostEqual(get_total_wealth(), expected_total, places=5, msg="Leaked in R4 (Bond Mature)")
