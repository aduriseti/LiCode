...
class LMSRMarket:
    ...
    def cost_function(q_yes: float, q_no: float, b: float) -> float:
        ...
        return b * (
            max_q / b + math.log(math.exp((q_yes - max_q) / b) + math.exp((q_no - max_q) / b))
        )
    ...
    def calculate_trade_cost(
        q_yes: float, q_no: float, b: float, delta_q: float, is_yes_share: bool
    ) -> float:
        """
    ...
    @staticmethod
    def calculate_delta_q(
        q_yes: float, q_no: float, b: float, cost: float, is_yes_share: bool
    ) -> float:
        """
        ...
        ln_sum_exp = max_q_inner / b + math.log(
            math.exp((old_q - max_q_inner) / b) + math.exp((fixed_q - max_q_inner) / b)
        )
