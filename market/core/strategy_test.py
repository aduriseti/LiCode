...
class StrategyTest(unittest.TestCase):
    ...
    def test_no_short_at_max_confidence(self):
        ...
        self.assertEqual(
            len(wagers), 0, "No wagers should be created when belief and price are at max threshold"
        )
    ...
    def test_no_long_at_min_confidence(self):
        ...
        wagers = Strategy.beliefs_to_wagers(beliefs, wealth, state)
        self.assertEqual(
            len(wagers), 0, "No wagers should be created when belief and price are at min threshold"
        )
