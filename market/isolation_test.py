...
class TestIsolationRegression(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        subprocess.run(
            ["git", "commit", "-m", "init"], cwd=self.repo_dir, check=True, capture_output=True
        )
    ...
    async def test_orchestrator_isolation_and_context(self):
        ...
        with mock.patch("os.getcwd", return_value=self.repo_dir):
            ...
            self.assertEqual(
                mode_parent, 0o711, f"worktrees_dir should be 0o711, got {oct(mode_parent)}"
            )
            ...
            self.assertEqual(
                file_mode, 0o600, f"Worktree files should be 0o600, got {oct(file_mode)}"
            )
        ...
        with mock.patch("asyncio.create_subprocess_exec") as mock_exec:
            ...
            with mock.patch("asyncio.open_connection", side_effect=mock_connect):
                ...
                try:
                    ...
                    self.fail(
                        f"Config validation failed (STRICT MODE). JSON: {config_json}. Error: {e}"
                    )
