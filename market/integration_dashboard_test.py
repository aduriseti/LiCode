...
class IntegrationDashboardTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_integration_flow_with_json_logs(
        self, MockOracle, MockClone, MockExec, MockAsyncSocket, MockSocket, MockShark
    ):
        """
        ...
        agent_0_action = AgentAction(
            ...
            proposals=[{"type": "VERIFIER", "path": "tests/v1"}],
        )
        # Agent 1 will just bet
        agent_1_action = AgentAction(
            agent_id="agent_1", beliefs={"agent_0_cand": 0.5, "agent_1_cand": 0.5}
        )
        ...
        with patch("sys.stdout", new_callable=io.StringIO) as mock_stdout:
            ...
            runner = MarketRunner(
                prompt="Implement Fibonacci", n_agents=2, budget=100.0, api_url="http://127.0.0.1"
            )
    ...
    async def test_recovery_from_malformed_llm_json(
        self, MockPort, MockServer, MockClient, MockClone
    ):
        """
...
class TestDashboardFormatting(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.state = MarketState(
            round_num=1, liquidity_b=100.0, whale_wealth=1000.0, prompt="Test Prompt"
        )
        self.state.assets = {
        ...
        self.state.agents = {"agent_0": AgentPortfolio(agent_id="agent_0", wealth=500.0)}
