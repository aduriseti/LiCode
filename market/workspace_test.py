...
class WorkspaceTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        subprocess.run(
            ["git", "init"], cwd=self.project_root, check=True, stdout=subprocess.DEVNULL
        )
        subprocess.run(
    ...
    async def test_hybrid_snapshot_cleanliness_and_safety(self):
        ...
        subprocess.run(
            ["git", "commit", "-m", "Initial Commit"], check=True, stdout=subprocess.DEVNULL
        )
        ...
        remotes = subprocess.run(
            ["git", "remote"], cwd=dest_dir, capture_output=True, text=True
        ).stdout.strip()
        ...
        diff_proc = subprocess.run(
            ["git", "diff", "--cached", "HEAD"], cwd=dest_dir, capture_output=True, text=True
        )
        ...
        self.assertEqual(
            diff_output.strip(), "", "Diff should be empty (Baseline captured uncommitted state)"
        )
    ...
    async def test_hybrid_snapshot_overlay(self):
        ...
        subprocess.run(
            ["git", "commit", "-m", "Add committed file"], check=True, stdout=subprocess.DEVNULL
        )
        ...
        # 3. Verify Content
        with open(os.path.join(dest_dir, "committed.py")) as f:
            content = f.read()
            self.assertIn("I am modified", content, "Uncommitted modifications should be present")

        self.assertTrue(
            os.path.exists(os.path.join(dest_dir, "new_file.py")),
            "New untracked file should be present",
        )

        # 4. Verify Baseline Commit
        log = subprocess.run(
            ["git", "log", "--oneline"], cwd=dest_dir, capture_output=True, text=True
        ).stdout
