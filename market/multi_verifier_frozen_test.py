...
from market.core.state import MarketState, MarketAsset, MarketBond
...
class TestMultiVerifierAndFrozenState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bond_deduplication(self):
        ...
        self.assertEqual(len(self.orch.state.bonds), 0, "Should not have created a redundant bond for an existing asset")
    ...
    async def test_frozen_state_wealth_isolation(self):
        ...
        vid = "v_old"
        self.orch.state.assets[vid] = MarketAsset(id=vid, type="VERIFIER", description="Old", q_yes=100)
        # Bond worth ~73
        self.orch.state.bonds.append(MarketBond("agent_0", vid, 100.0, unlock_round=self.orch.state.round_num + 1))
        ...
        with mock.patch("market.core.strategy.Strategy.beliefs_to_wagers", wraps=Strategy.beliefs_to_wagers) as spy_kelly:
            with mock.patch.object(self.orch, '_run_oracle', new_callable=mock.AsyncMock):
                await self.orch.process_round([action])
            ...
            self.assertEqual(passed_wealth, initial_wealth, "Kelly should use frozen wealth from round start")
            self.assertNotEqual(passed_wealth, agent.wealth, "Kelly should not see the matured bond wealth yet")
