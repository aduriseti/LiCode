...
class TestBankruptcyLogic(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_agent_survives_with_bonds(self):
        ...
        self.state.assets[vid] = MarketAsset(
            id=vid, type="VERIFIER", description="Test", q_yes=100.0
        )
        ...
        self.state.bonds.append(
            MarketBond(agent_id=self.agent_id, asset_id=vid, q_shares=100.0, unlock_round=10)
        )
    ...
    async def test_agent_bankrupts_when_total_value_low(self):
        ...
        vid = "v1"
        self.state.assets[vid] = MarketAsset(
            id=vid, type="VERIFIER", description="Test", q_yes=0.0, q_no=0.0
        )
        # Price is 0.5

        self.state.bonds.append(
            MarketBond(agent_id=self.agent_id, asset_id=vid, q_shares=10.0, unlock_round=10)
        )
