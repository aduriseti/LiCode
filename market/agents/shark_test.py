...
from opencode_ai import APITimeoutError
...
from market.agents.shark import FatalAgentError, Shark
...
class SharkTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_format_state_prompt_content(
        self, mock_file, mock_listdir, mock_isdir, mock_exec, mock_shell, mock_run
    ):
        # Setup State
    ...
    async def test_format_state_prompt_no_error_on_valid_diff(
        self, mock_isdir, mock_exec, mock_shell
    ):
...
class TestSharkActionParsing(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_parsing_failure_no_json(self, MockClient):
        ...
        self.setup_mock_stream(
            MockClient, "I am thinking about Fibonacci but I will not give you JSON today."
        )
    ...
    async def test_parsing_malformed_json_recovery(self, MockClient):
        ...
        self.setup_mock_stream(
            MockClient,
            'Here is a list: { "item": 1 } and here is the real answer: {"beliefs": {"cand_0": 0.5}}',
        )
...
class TestSharkRetryLogic(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_action_retry_on_parsing_error(self, mock_sleep, MockClient):
        ...
        with patch.object(Shark, "_chat_with_network_retry") as mock_chat:
            mock_chat.side_effect = ["Not JSON", '{"beliefs": {"cand_0": 0.7}}']
    ...
    async def test_get_action_max_retries_exceeded(self, mock_sleep, MockClient):
        ...
        with patch.object(Shark, "_chat_with_network_retry") as mock_chat:
            mock_chat.side_effect = [APITimeoutError("Timeout 1"), APITimeoutError("Timeout 2")]
    ...
    async def test_chat_with_network_retry_respects_timeout_config(self, MockClient):
        ...
        dummy_part = TextPart(
            id="p1", messageID="m1", sessionID="s1", type="text", text='{"beliefs": {}}'
        )
        ...
        self.assertEqual(
            timeouts.count(call_timeout),
            2,
            f"Should have 2 clients with timeout {call_timeout}. Found: {timeouts}",
        )
    ...
    async def test_get_action_fatal_failure_on_short_timeout(self, mock_sleep, MockClient):
        ...
        with patch.object(
            Shark, "_chat_with_network_retry", side_effect=APITimeoutError("Timeout")
        ):
            state = MarketState(round_num=1, liquidity_b=100.0)
            from market.agents.shark import FatalAgentError
