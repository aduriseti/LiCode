...
from opencode_ai import APIConnectionError, APITimeoutError, AsyncOpencode
from opencode_ai.types import TextPart, ToolPart, ToolStateCompleted, ToolStateRunning
...
class Shark:
    ...
    def __init__(self, agent_id: str, **kwargs):
        ...
        self.model = kwargs.get("model", "gemini-3-flash")
        self.provider = kwargs.get("provider", "opencode")
        self.log_path = kwargs.get("log_path")
        self.trace_path = kwargs.get("trace_path")
        self.timeout = kwargs.get("timeout", 300.0)
        self.max_retries = kwargs.get("max_retries", 3)
        self.initial_backoff = kwargs.get("initial_backoff", 120.0)
        self.max_backoff = kwargs.get("max_backoff", 1000.0)
        api_url = kwargs.get("api_url", "http://127.0.0.1:4096")

        # Initialize Async OpenCode client
        self.client = AsyncOpencode(base_url=api_url, timeout=self.timeout, max_retries=0)
        self.session = None
    ...
    async def initialize_session(self):
        ...
        if not self.session:
            ...
            if not hasattr(self.session, "id"):
                raise RuntimeError(
                    f"Failed to create session for Shark {self.agent_id}. Got: {self.session}"
                )
    ...
    def _log_interaction(self, prompt: str, response: Any, error: str | None = None):
        # Log to file
    ...
    def _build_system_prompt(self) -> str:
        ...
        return f"""You are Agent {self.agent_id}, a strategic software engineer participating in \
a Logical Induction Market Tournament.
Your Goal: Win the tournament by producing the project that BEST satisfies the User's Problem \
Statement and profiting from accurate predictions.
Your assigned candidate solution is the ENTIRE worktree "{cid}".

**Session Management Policy:**
If your session is interrupted due to a timeout, it is likely because the system detected \
an unproductive loop or excessive tool use.
In such cases, the system will automatically retry with an increased time limit \
(initial: {self.timeout}s, scaling up to: {self.max_backoff}s).
Your session will be interrupted (equivalent to pressing Escape twice) during timeouts \
to restore responsiveness.

**The Environment:**
- **Read Access:** You can ONLY read files in your own worktree ("{cid}"). You do NOT have \
direct read/exec access to rival worktrees; instead, you are provided with **Diff Summaries** \
of their changes in the Market Assets section below to analyze their solutions.
- **Write Access:** You can ONLY modify your own worktree ("{cid}").
- **Modifications:** You have direct write access. Use your tools (e.g. `write_file`) to \
edit your code. Do NOT return file content in your JSON response.

**The Market & Verification:**
- **Verifiers:** You propose tests (`VERIFIER` packages) to prove your code works or expose \
bugs in others.
...
- **Execution:** The system will copy that directory to the central registry and run it \
against all candidates.
    ...
    **Implication:** Your `run.sh` executes **inside** a copy of the candidate's project. \
You can access their `main.py` directly as `./main.py`.
- **Scope:** Verifiers can test Logic, Performance, and Stability.
- **Self-Verification:** Your verifier MUST pass on your own candidate! If it fails, you \
look incompetent.
...
"""
        ...
        try:
            ...
            for attempt in range(max_total_attempts + 3):
                try:
                    ...
                    # Call API
                    content = await self._chat_with_network_retry(
                        current_prompt, timeout=current_timeout
                    )
                    ...
                    if retry_count > self.max_retries:
                        logging.error(
                            f"Shark {self.agent_id} network/timeout failed after {self.max_retries} retries: {e}"
                        )
                    ...
                    if isinstance(e, APITimeoutError):
                        logging.warning(
                            f"Shark {self.agent_id} timed out ({current_timeout}s). Interrupting and retrying immediately with {next_timeout}s limit (Retry {retry_count}/{self.max_retries})"
                        )
                        ...
                        logging.warning(
                            f"Shark {self.agent_id} connection error: {e}. Retrying immediately (Retry {retry_count}/{self.max_retries})"
                        )
                    ...
                    if attempt >= max_total_attempts + 2:
                        logging.error(
                            f"Shark {self.agent_id} failed parsing after multiple correction attempts: {e}"
                        )
                        raise FatalAgentError(
                            f"Shark {self.agent_id} failed to provide valid JSON: {e}"
                        )

                    logging.warning(
                        f"Shark {self.agent_id} parsing failed. Sending error back for correction (Attempt {attempt + 1})"
                    )
                    current_prompt = f"ERROR: Your previous response was invalid: {e!s}\nPlease provide your updated beliefs and proposals in the correct JSON format."
    ...
    async def _chat_with_network_retry(self, prompt: str, timeout: float | None = None) -> str:
        ...
        try:
            ...
            if not messages:
                raise FatalAgentError(
                    f"No messages found for Shark {self.agent_id} after chat call."
                )
            ...
            if not content:
                raise FatalAgentError(
                    f"Received empty response content from Shark {self.agent_id}."
                )
        ...
        try:
            ...
            if data is None:
                raise LLMResponseError(
                    f"Response missing 'beliefs' or 'proposals' keys. Content sample: {content[:100]}..."
                )
            ...
            if isinstance(e, LLMResponseError):
                raise e
            raise LLMResponseError(f"Invalid JSON format: {e!s}")
    ...
    async def _format_state_prompt(self, state: MarketState) -> str:
        ...
        for aid, asset in state.assets.items():
            ...
            if asset.type == "CANDIDATE":
                ...
                failures = [
                    v_id
                    for key, failed in state.test_failures.items()
                    if failed and key.endswith(f":{aid}")
                    for v_id in [key.split(":")[0]]
                ]
                if failures:
                        ...
                        if current_file_diff:
                            if len(current_file_diff) > 100:
                                ...
                                processed_diff.append(
                                    f"... (Truncated {len(current_file_diff) - 100} lines) ..."
                                )
                            else:
