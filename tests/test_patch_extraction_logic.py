...
class TestPatchExtraction(unittest.TestCase):
    def test_agent_internal_commits_do_not_break_diff(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            subprocess.run(["git", "add", "."], cwd=temp_dir, check=True, capture_output=True)
            subprocess.run(
                ["git", "commit", "-m", "Agent internal commit"],
                cwd=temp_dir,
                check=True,
                capture_output=True,
            )
            ...
            self.assertIn(
                "--- /dev/null", patch, "Patch should show new file addition using /dev/null"
            )
            self.assertIn("+++ b/agent_test.py", patch, "Patch should include agent_test.py")
            self.assertIn(
                "agent test v2 - modified", patch, "Patch should contain the final content"
            )
            ...
            self.assertNotIn(
                "--- a/agent_test.py",
                patch,
                "Patch should NOT show agent_test.py as a modification of an existing file",
            )
